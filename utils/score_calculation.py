...
def get_ood_scores_gradnorm(loader, net, ood_num_examples, num_classes, T):
    def grad_norm(param):
        return torch.sum(torch.abs(param)).cpu().numpy()

    confs = []
    logsoftmax = torch.nn.LogSoftmax(dim=-1).cuda()
    new_loader = torch.utils.data.DataLoader(loader.dataset, batch_size=1, shuffle=True,
                                             num_workers=4, pin_memory=True)
    for sample_idx, (data, target) in enumerate(tqdm(new_loader)):
        if sample_idx >= ood_num_examples:
            break
        inputs = Variable(data.cuda(), requires_grad=True)
        net.zero_grad()
        outputs = net(inputs)
        outputs = outputs / T
        targets = torch.ones((data.shape[0], num_classes)).cuda()
        loss = torch.mean(-targets * logsoftmax(outputs))

        loss.backward()

        #first_layer_grad = net.conv1.weight.grad.data
        #layer_grad_norm = grad_norm(first_layer_grad)
        #for param in net.block1.parameters():
        #    layer_grad_norm += grad_norm(param.grad.data)
